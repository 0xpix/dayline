...
object NotificationScheduler {
    ...
    fun cancel(context: Context, item: DaylineItem) {
        ...
        NotificationManagerCompat.from(context).cancel(requestCode(item.id))
    }
    ...
    fun scheduleNext(
        ...
        cancelAlarmOnly(context, item.id)
        ...
        scheduleAlarm(context, triggerMillis, pendingIntent(context, item.id))
    }
    ...
    private fun scheduleAlarm(
        ...
        // Permission can change between canScheduleExactAlarms() and the call.
        // Fall back instead of letting a SecurityException silently kill the
        // reminder resync path.
    ...
    private fun cancelAlarmOnly(context: Context, itemId: String) {
        val manager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        manager.cancel(pendingIntent(context, itemId))
    }
