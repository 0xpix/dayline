...
object GithubBetaUpdater {
    ...
    suspend fun check(currentVersion: String = BuildConfig.VERSION_NAME): UpdateUiState = withContext(Dispatchers.IO) {
            ...
            val parsed = buildList {
                for (index in 0 until releases.length()) {
                    ...
                    val apkCandidates = buildList {
                        for (assetIndex in 0 until assets.length()) {
                            ...
                            if (name.endsWith(".apk", ignoreCase = true) && url.isNotBlank()) {
                                add(ApkAsset(name, url, asset.optLong("size", -1L).takeIf { it >= 0L }))
                            }
                        }
                    }
                    val selected = apkCandidates.firstOrNull { it.name.contains(tag, ignoreCase = true) }
                        ?: apkCandidates.firstOrNull { it.name.contains("beta", ignoreCase = true) }
                    ...
                    if (selected != null) {
                        for (assetIndex in 0 until assets.length()) {
                            val asset = assets.optJSONObject(assetIndex) ?: continue
                            if (asset.optString("name") == "${selected.name}.sha256") {
                                checksumUrl = asset.optString("browser_download_url").takeIf { it.isNotBlank() }
                                break
                            }
                        }
                    }
    ...
    suspend fun download(context: Context, release: BetaRelease): Result<File> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            release.checksumUrl?.let { checksumUrl ->
                ...
                check(expected.length == 64 && expected == actual) { "Downloaded APK checksum did not match the GitHub release" }
            }
    ...
    private fun friendlyError(error: Throwable): String = when (error) {
        ...
        else -> error.message?.takeIf { it.isNotBlank() } ?: "Couldn't check for updates."
    }
    ...
    private fun safeFileName(name: String): String = name.replace(Regex("[^A-Za-z0-9._-]"), "_").ifBlank { "dayline-beta.apk" }
}
